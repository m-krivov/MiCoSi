...
TiXmlElement *Serializer::SerializeCellConfiguration(Cell *cell, uint32_t initialRandSeed, MemoryStream* stream)
{
  ...
  try
  {
    ...
    if (res->LinkEndChild(params) == NULL)
      throw std::runtime_error("Internal error at Serializer::SerializeCellConfiguration() #1");

    params->SetAttribute("init_rand_seed", initialRandSeed);
...
TiXmlElement *Serializer::SerializeTimeLayer(Cell *cell, double time, uint32_t randSeed, MemoryStream* stream)
{
  TiXmlElement *res = NULL;
  try
  {
    ...
    // res->SetDoubleAttribute("t", time);
    res->SetAttribute("rand", randSeed);
...
TiXmlElement *Serializer::SerializeSimParams(SimParams *params, MemoryStream* stream)
{
  TiXmlElement *res = NULL;
