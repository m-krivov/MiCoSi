...
void FileExplorer::WritingChunk::Add(TiXmlElement* elem, double time, MemoryStream* stream)
{
  ...
  {
    _time = time;
    if (_doc->LinkEndChild(new TiXmlDeclaration("1.0", "utf-8", "no")) == nullptr)
      throw std::runtime_error("Cannot create XML declaration");
    TiXmlElement *root = new TiXmlElement("Chunk");
    if (_doc->LinkEndChild(root) == nullptr)
    {
      delete root;
  ...
  if (_doc->RootElement()->LinkEndChild(elem) == nullptr)
...
void FileExplorer::ReadingChunk::LoadChunk(std::shared_ptr<Chunk> chunk)
{
  ...
  if(_doc->RootElement() == nullptr || _doc->RootElement()->FirstChild() == nullptr)
...
FileExplorer::FileExplorer(const std::string &file)
{
  ...
  _writingChunk = nullptr;
  _readingChunk = nullptr;
  _simParams.clear();
  _additional.clear();
...
void FileExplorer::SaveService(TiXmlElement* elem, MemoryStream* stream)
{
  ...
  if (doc->LinkEndChild(new TiXmlDeclaration("1.0", "utf-8", "no")) == nullptr)
  {
    delete doc;
    throw std::runtime_error("Cannot create XML declaration");
  }
  ...
  if (doc->LinkEndChild(root) == nullptr)
  {
    delete doc;
    delete root;
    throw std::runtime_error("Cannot create root section");
  ...
  if (root->LinkEndChild(elem) == nullptr)
  {
    delete doc;
    throw std::runtime_error("Cannot link element");
  }
...
FileExplorer::ChunkElement* FileExplorer::LoadService(size_t idx)
{
  ...
  try
  {
    ...
    if(doc->RootElement() == nullptr || doc->RootElement()->FirstChild() == nullptr ||
      doc->RootElement()->FirstChild()->ToElement() == nullptr)
