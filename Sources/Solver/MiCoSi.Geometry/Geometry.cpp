...
{
  // Get intersection with rectangle-defined plane
  if (!AreIntersected(seg, Plane(rect.p, CrossProduct(rect.v1, rect.v2).Normalize()), ipoint))
  { return false; }

  // Check that intersection point located inside rectangle
...
{
  // Get intersection with circle-defined plane
  if (!AreIntersected(seg, Plane(circle.p, CrossProduct(circle.r1, circle.r2).Normalize()), ipoint))
  { return false; }

  // Check that intersection point located inside semicircle
