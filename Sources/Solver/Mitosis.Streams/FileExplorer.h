...
public:
  class ChunkElement
  {
    ...
    public:
      ...
      inline TiXmlElement* XmlElement()
      { return _elem; }
      inline void* BinDataPointer()
      { return _data; }
      inline uint64_t SizeInBytes()
      { return _sizeInBytes; }
      ~ChunkElement();
...
public:
  ...
  inline ChunkElement* Configuration()
  ...
  static FileExplorer *Create(const char *file, size_t elemPerChunk, uint64_t version);

  // Opens previously saved results for exploring.
  static FileExplorer *Open(const char *file, size_t elemPerChunk = 10);

  static FileExplorer *Repair(const char *file, FrameExtractor fextr, ServiceExtractor sextr,
