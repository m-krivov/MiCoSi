...
void Cell::CreateObjects(CellData *data)
{
  ...
  {
    _MTs.resize(_data->MTsPerPole() * 2, nullptr);
  ...
  {
    _chromosomes.resize(_data->ChromosomePairs() * 2, nullptr);
    for (uint32_t i = 0; i < _chromosomes.size(); i++)
    { _chromosomes[i] = new Chromosome(i, this, _data.get()); }

    _chromosomePairs.resize(_data->ChromosomePairs(), nullptr);
    _springs.resize(_data->ChromosomePairs(), nullptr);
