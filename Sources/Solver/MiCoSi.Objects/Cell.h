...
class Cell : public ICell, public IClonnable, private ICellObjectProvider
{
  ...
  private:
    ...
    virtual Pole *GetPole(uint32_t ID) const override
    { return _poles[ID]; }

    virtual MT *GetMT(uint32_t ID) const override
    { return _MTs[ID]; }

    virtual Chromosome *GetChromosome(uint32_t ID) const override
    { return _chromosomes[ID]; }

    virtual ChromosomePair *GetChromosomePair(uint32_t ID) const override
    { return _chromosomePairs[ID]; }

    virtual Spring *GetSpring(uint32_t ID) const override
    { return AreSpringsBroken() ? nullptr : _springs[ID]; }
