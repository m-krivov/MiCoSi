...
uint64_t Serializer::SerializeVersion(const Version &programVersion,
                                      const std::string &flags,
                                      int fileFormatVersion)
{
  // A temporary fix, will be replaced after introducing new containers
  uint16_t iflags = 0;
  if (flags.find("MICOSI_PRECISION_FP64", 0) != std::string::npos)
  { iflags |= 1; }
  if (flags.find("MICOSI_RNG_MTG", 0) != std::string::npos)
  { iflags |= 2; }
                                                     ...
                                                     MemoryStream &stream)
{
  ...
  try
  {
    ...
    // Add static information about MTs
                                             ...
                                             const Random::State &rng, MemoryStream &stream)
{
  ...
  try
  {
    ...
    if (res->LinkEndChild(cellSect) == nullptr)
    ...
    for (size_t i = 0; i < lengthes.size(); i++)
    {
      ...
      boundIDs[i] = mtsRefs[i]->BoundChromosome() == nullptr ? -1 : mtsRefs[i]->BoundChromosome()->ID();
    }
    ...
    if (res->LinkEndChild(chrs) == nullptr)
