...
std::string SimParamsFormatter::ExportAsProps(SimParams *params)
{
...

  {
    ...
    for (size_t i = 0; i < intParams.size(); i++)
      ss << SimParameter::Int::Info(intParams[i]).Name()
         << "=" << params->GetParameter(intParams[i]) << std::endl;
  }

  {
    const std::vector<SimParameter::Double::Type> &doubleParams = SimParameter::Double::All();
    for (size_t i = 0; i < doubleParams.size(); i++)
      ss << SimParameter::Double::Info(doubleParams[i]).Name()
         << "=" << params->GetParameter(doubleParams[i]) << std::endl;
  }
...
void SimParamsFormatter::ImportAsProps(SimParams *params, const std::string &props)
{
  ...
  // Store them as name + value pairs
  ...
  do
  {
    ...
    // Step 1. Make some clean up - removing multiple spaces, asm-styled comments and '\n' plus '\r' symbols.
    std::string str;
    ...
    // Step 4. Add records to vector for future import operation
    records.push_back(std::make_pair(param, value));
  }
  while (!ss.eof());

  // Import parameters. Correctness will be checked there
  try
